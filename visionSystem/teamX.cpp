...
Mat findCandidateLP(Mat src)
{
	...
	findContours(cannyImage, contours, hierarchy, CV_RETR_TREE, CV_CHAIN_APPROX_SIMPLE, Point());	//윤곽선정보를 contours에 담음.
	...
	for(int i = 0; i< contours.size(); i++){
		...
		if((ratio > 1.2 && ratio<6.0  && boundRect[i].area()>1000 && boundRect[i].area()<15000)){		//세로 대비 가로 비율이 2.7 이상과 일정 면적을 만족하면 번호판 후보로 선정
...
Mat selectCandidateLP(Mat src, Mat preProcImage, CvRect &fRect)
{
	...
	for(int i = 0; i< contours.size(); i++){
        ...
        if((ratio <= 3.0) && (ratio >= 1) && (boundRect[i].area() <= 800)&& (boundRect[i].area() > 100)){	//숫자의 윤곽선으로 판단되는 윤곽선 추출
	...
	for(int u=0;u<boundRect2.size();u++)
	{
		...
		for(int v=u+1;v<boundRect2.size();v++)
		{
			...
			if(distance_x > 150)  //숫자와 숫자사이 간격이 너무 먼 경우 번호판 윤곽선이 아니라고 판단
                break;

			if(distance_x==0)
				distance_x=1;
			interVal[v-u-1]=median_v-median_v_pre;	//숫자사이의 거리를 담음.
			...
			if(gradient<0.25 &&interVal[v-u-1]<=50)	//숫자 윤곽선이 수평을 이루고, 겹친 숫자 없는지 판별
			{
		...
		if((countRealNumber>=6)&&(distance_x_stdev<15) && (distance_x_stdev>=0))	//번호판의 개수가 6개이상(겹친 숫자가 존재가능)이고, 숫자사이 간격의 표준편차가 10 이하인 경우 바로 정답
		{
